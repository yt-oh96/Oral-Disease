...
import time
import csv
from src.utils import eval_metrics, auc_score, t
...
def train(model,n_cls, n_epochs, trainloader,valloader, criterion, optimizer, scheduler,tri ,device):
    ...
    for epoch in range(n_epochs):
        ...
        if epoch == 0 or epoch == 15 or epoch == 16 or epoch == 17:
            print(epoch, 'save weight')
            best_epoch = epoch
            best_f1 = valid_f1
            best_model = deepcopy(model)
            torch.save(model.state_dict(), '/USER/INFERENCE/CANCER/weights/final_model_epoch'+str(epoch)+'.pth')
            pass
